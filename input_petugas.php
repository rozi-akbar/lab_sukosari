...
if (!empty($_POST["submit"])) {
  $query = "UPDATE tbl_petugas SET nip='" . $_POST["nip"] . "', nama='" . $_POST["nama"] . "' WHERE nip='" . $_POST["nip"] . "' ";
  $result = $db_handle->executeQuery($query);
  if (!$result) {
    $message = "Problem in Update to database. Please Retry.";
  } else {
    echo '<script>alert("Berhasil Dirubah")</script>'; 
    header("Location:input_petugas.php");
  }
}
$result = $db_handle->runQuery("SELECT * FROM tbl_petugas");
require('header.php');
?>
